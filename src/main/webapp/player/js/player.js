...
class Musics {
    ...
    constructor() {
        ...
        $.ajax({
            type:"post",
            url:"musicList?sheetid="+sheetid,
            async:false,
            date:'',
            // success:function(msg){
            //     this.songs = msg.musiclist
            //     // console.log("1="+JSON.stringify(this))
            // }
        })
            .done(function (msg){
                // this.songs = msg.musiclist
        ...
        [
            {
                ...
                songUrl: './songs/Campsite Dream - Driving Home for Christmas.mp3',
...
class PlayerCreator {
    constructor() {
        ...
        //工具
        this.util = new Util();
        this.musics = new Musics(); //电影信息
        this.song_index = 0; // 当前播放的电影索引
        this.loop_mode = 0; // 1 2
        // 下方电影列表容器
        this.song_list = $('.music__list_content');

        this.render_doms = { //切换电影时需要渲染的dom组
    ...
    bindEventListener() {
        ...
        //电影进度 this.audio.duration

        //可以播放的时候触发（电影的基本信息都已经获取到了）
        this.audio.oncanplay = () => {
            //避免重复实例化
            if (this.progress) {
                this.progress.max = this.audio.duration; //切换电影后更新时长
